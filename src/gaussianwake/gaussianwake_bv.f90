...
! linearized gaussian magnitude term for near wake
    ...
    temp17 = COS(yaw)
    DO nd=1,nbdirs
      tempb13(nd) = EXP(-(0.5_dp*temp23))*deltavb(nd)
      tempb14(nd) = temp26*tempb13(nd)
      tempb15(nd) = (deltav0m-deltavdm)*tempb14(nd)/x0
      tempb16(nd) = -(0.5_dp*EXP(-(0.5_dp*temp25))*temp27*tempb13(nd)/&
&       temp24)
      tempb17(nd) = -(0.5_dp*EXP(-(0.5_dp*temp23))*temp27*temp26*deltavb&
&       (nd)/temp22)
      deltav0mb(nd) = temp28*tempb14(nd)
      deltavdmb(nd) = (1.0_8-temp28)*tempb14(nd)
      xb(nd) = tempb15(nd)
      x0b(nd) = -(temp28*tempb15(nd))
      deltayb(nd) = 2*deltay*tempb16(nd)
      sigmay_spreadb(nd) = -(temp25*2*sigmay_spread*tempb16(nd))
      deltazb(nd) = 2*deltaz*tempb17(nd)
      sigmaz_spreadb(nd) = -(temp23*2*sigmaz_spread*tempb17(nd))
      IF (1.0_dp - temp21*temp20 .EQ. 0.0) THEN
        tempb18(nd) = 0.0
      ELSE
        tempb18(nd) = -(deltavdmb(nd)/(2.0*SQRT(1.0_dp-temp21*temp20)))
      END IF
      tempb19(nd) = -(temp21*tempb18(nd)/temp18)
      tempb20(nd) = -(temp20*tempb19(nd))
      sigmay_db(nd) = sigmaz_d*8.0_dp*tempb20(nd)
      sigmaz_db(nd) = 8.0_dp*sigmay_d*tempb20(nd)
      IF (1.0_dp - temp17*temp16 .EQ. 0.0) THEN
        tempb21(nd) = 0.0
      ELSE
        tempb21(nd) = -(deltav0mb(nd)/(2.0*SQRT(1.0_dp-temp17*temp16)))
      END IF
      yawb(nd) = yawb(nd) + temp16*SIN(yaw)*tempb21(nd) + temp20*SIN(yaw&
&       )*tempb18(nd)
      tempb22(nd) = -(temp17*tempb21(nd)/temp14)
      ctb(nd) = ctb(nd) + rotor_diameter_ust**2*tempb22(nd) + &
&       rotor_diameter_ust**2*tempb19(nd)
      rotor_diameter_ustb(nd) = rotor_diameter_ustb(nd) + ct*2*&
&       rotor_diameter_ust*tempb22(nd) + ct*2*rotor_diameter_ust*tempb19&
&       (nd)
      tempb23(nd) = -(temp16*tempb22(nd))
      sigmay_0b(nd) = sigmaz_0*8.0_dp*tempb23(nd)
      sigmaz_0b(nd) = 8.0_dp*sigmay_0*tempb23(nd)
    END DO
    discontinuity_pointb(:) = 0.0_8
  ELSE
    STOP
