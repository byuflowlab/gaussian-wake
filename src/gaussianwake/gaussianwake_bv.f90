...
! out
  ...
  ELSE
    ...
    temp1 = temp0/temp
    DO nd=1,nbdirs
      tempb3(nd) = LOG(temp9)*wake_offsetb(nd)
      tempb4(nd) = b*c*d*wake_offsetb(nd)/(temp9*g)
      ab(nd) = wake_offsetb(nd)
      bb(nd) = d*c*tempb3(nd)
      cb(nd) = d*b*tempb3(nd)
      db(nd) = b*c*tempb3(nd)
      fb(nd) = tempb4(nd)
      gb(nd) = -(temp9*tempb4(nd))
...
&       14.7_dp + temp2*2*rotor_diameter*tempb2(nd)
      IF (temp1 .EQ. 0.0) THEN
        tempb(nd) = 0.0
      ELSE
        tempb(nd) = cb(nd)/(2.0*SQRT(temp1)*temp)
      END IF
      yawb(nd) = yawb(nd) - SIN(yaw)*tempb(nd) - rotor_diameter**2*SIN(&
&       yaw)*tempb2(nd)
      tempb0(nd) = -(temp1*tempb(nd))
      IF (1.0_dp - ct .EQ. 0.0) THEN
        ctb(nd) = ctb(nd) + ky*kz*tempb0(nd) - db(nd)
      ELSE
        ctb(nd) = ctb(nd) + ky*kz*tempb0(nd) + ((-1.0)-1.3_dp/(2.0*SQRT(&
&         1.0_dp-ct)))*db(nd)
      END IF
      kyb(nd) = kyb(nd) + ct*kz*tempb0(nd)
      kzb(nd) = kzb(nd) + ct*ky*tempb0(nd)
...
! print *, "deltav 2014 = ", deltav
    ...
    DO nd=1,nbdirs
      IF (1.0_dp - temp11 .EQ. 0.0) THEN
        tempb0(nd) = 0.0
      ELSE
        tempb0(nd) = EXP(-temp8)*deltavb(nd)/(2.0*temp12*temp10)
      END IF
      tempb1(nd) = -(8.0_dp*temp11*2*(temp9+epsilon_2014)*tempb0(nd))
      tempb2(nd) = tempb1(nd)/rotor_diameter_ust
      tempb3(nd) = -(EXP(-temp8)*(1.0_dp-temp12)*deltavb(nd)/temp2)
      tempb4(nd) = 2*temp6*tempb3(nd)/temp5
      tempb5(nd) = 2*temp4*tempb3(nd)/temp3
      tempb6(nd) = -(2.0_dp*temp8*2*(temp1+epsilon_2014)*tempb3(nd))
...
&       nd) - temp4*wec_factor*tempb5(nd) - temp6*wec_factor*tempb4(nd) &
&       - temp9*tempb2(nd)
...
&         /(2.0*temp**2)-1.0/(2.0*temp0))*tempb(nd)
    END DO
    sigmayb(:) = 0.0_8
    sigmazb(:) = 0.0_8
...
! local
  ...
  REAL(dp), DIMENSION(nbdirs) :: deltav0mb, deltavdmb
! only for 2014 version
  REAL(dp) :: beta_2014, epsilon_2014
  REAL(dp), DIMENSION(nbdirs) :: beta_2014b, epsilon_2014b
! out
...
! linearized gaussian magnitude term for near wake
      ...
      temp0 = SQRT(-ct + 1.0_dp)
      DO nd=1,nbdirs
        tempb6(nd) = EXP(-temp12)*deltavb(nd)
        tempb7(nd) = (deltav0m-deltavdm)*tempb6(nd)/x0
        tempb8(nd) = -(EXP(-temp12)*((deltav0m-deltavdm)*temp13+deltavdm&
&         )*deltavb(nd)/temp6)
        tempb9(nd) = 2*temp10*tempb8(nd)/temp9
        tempb10(nd) = 2*temp8*tempb8(nd)/temp7
        tempb11(nd) = -(2.0_dp*temp12*2*(temp5+epsilon_2014)*tempb8(nd))
        tempb12(nd) = tempb11(nd)/rotor_diameter_ust
        deltav0mb(nd) = temp13*tempb6(nd)
        deltavdmb(nd) = (1.0_8-temp13)*tempb6(nd)
        xb(nd) = k_2014*tempb12(nd) + tempb7(nd)
        deltazb(nd) = tempb9(nd)
        deltayb(nd) = tempb10(nd)
        IF (1.0_dp - ct/temp4 .EQ. 0.0) THEN
          tempb3(nd) = 0.0
        ELSE
          tempb3(nd) = deltavdmb(nd)/(2.0*SQRT(1.0_dp-ct/temp4)*temp4)
        END IF
        tempb4(nd) = -(8.0_dp*ct*2*(temp3+epsilon_2014)*tempb3(nd)/temp4&
&         )
        tempb5(nd) = tempb4(nd)/rotor_diameter_ust
        discontinuity_pointb(nd) = k_2014*tempb5(nd)
        IF (1.0_dp - ct/temp2 .EQ. 0.0) THEN
          tempb0(nd) = 0.0
        ELSE
          tempb0(nd) = deltav0mb(nd)/(2.0*SQRT(1.0_dp-ct/temp2)*temp2)
        END IF
        ctb(nd) = ctb(nd) + tempb0(nd) + tempb3(nd)
        tempb1(nd) = -(8.0_dp*ct*2*(temp1+epsilon_2014)*tempb0(nd)/temp2&
&         )
        epsilon_2014b(nd) = tempb4(nd) + tempb1(nd) + tempb11(nd)
        tempb2(nd) = tempb1(nd)/rotor_diameter_ust
        x0b(nd) = k_2014*tempb2(nd) - temp13*tempb7(nd)
        rotor_diameter_ustb(nd) = rotor_diameter_ustb(nd) - temp3*tempb5&
&         (nd) - temp1*tempb2(nd) - temp5*tempb12(nd) - temp8*&
&         wec_factor_2014*tempb10(nd) - temp10*wec_factor_2014*tempb9(nd&
&         )
        k_2014b(nd) = k_2014b(nd) + discontinuity_point*tempb5(nd) + x0*&
&         tempb2(nd) + x*tempb12(nd)
        IF (beta_2014 .EQ. 0.0) THEN
          beta_2014b(nd) = 0.0
        ELSE
          beta_2014b(nd) = 0.2_dp*epsilon_2014b(nd)/(2.0*SQRT(beta_2014)&
...
&           1.0_dp)/(2.0*temp**2)-1.0/(2.0*temp0))*tempb(nd)
      END DO
      sigmay_db(:) = 0.0_8
      sigmaz_0b(:) = 0.0_8
      sigmay_0b(:) = 0.0_8
...
&         deltavb(nd)/temp22)
      ...
      sigmay_0_spreadb(:) = 0.0_8
    ELSE
      temp29 = sigmaz_0_spread**2
      temp30 = deltaz**2/temp29
      temp31 = sigmay_0_spread**2
      temp32 = deltay**2/temp31
      temp33 = EXP(-(0.5_dp*temp32))
      temp35 = x/x0
      temp34 = (deltav0m-deltavdm)*temp35 + deltavdm
      DO nd=1,nbdirs
        tempb24(nd) = EXP(-(0.5_dp*temp30))*deltavb(nd)
        tempb25(nd) = temp33*tempb24(nd)
        tempb26(nd) = (deltav0m-deltavdm)*tempb25(nd)/x0
        tempb27(nd) = -(0.5_dp*EXP(-(0.5_dp*temp32))*temp34*tempb24(nd)/&
&         temp31)
        tempb28(nd) = -(0.5_dp*EXP(-(0.5_dp*temp30))*temp34*temp33*&
&         deltavb(nd)/temp29)
        deltav0mb(nd) = temp35*tempb25(nd)
        deltavdmb(nd) = (1.0_8-temp35)*tempb25(nd)
        xb(nd) = tempb26(nd)
        x0b(nd) = -(temp35*tempb26(nd))
        deltayb(nd) = 2*deltay*tempb27(nd)
        sigmay_0_spreadb(nd) = -(temp32*2*sigmay_0_spread*tempb27(nd))
        deltazb(nd) = 2*deltaz*tempb28(nd)
        sigmaz_0_spreadb(nd) = -(temp30*2*sigmaz_0_spread*tempb28(nd))
      END DO
      sigmay_spreadb(:) = 0.0_8
      sigmaz_spreadb(:) = 0.0_8
    END IF
    ...
    DO nd=1,nbdirs
      IF (1.0_dp - temp17*temp16 .EQ. 0.0) THEN
        tempb13(nd) = 0.0
      ELSE
        tempb13(nd) = -(deltav0mb(nd)/(2.0*SQRT(1.0_dp-temp17*temp16)))
      END IF
      tempb14(nd) = -(temp17*tempb13(nd)/temp14)
      IF (1.0_dp - temp21*temp20 .EQ. 0.0) THEN
        tempb16(nd) = 0.0
      ELSE
        tempb16(nd) = -(deltavdmb(nd)/(2.0*SQRT(1.0_dp-temp21*temp20)))
      END IF
      tempb17(nd) = -(temp21*tempb16(nd)/temp18)
      tempb18(nd) = -(temp20*tempb17(nd))
      yawb(nd) = yawb(nd) + temp16*SIN(yaw)*tempb13(nd) + temp20*SIN(yaw&
&       )*tempb16(nd)
      ctb(nd) = ctb(nd) + rotor_diameter_ust**2*tempb14(nd) + &
&       rotor_diameter_ust**2*tempb17(nd)
      rotor_diameter_ustb(nd) = rotor_diameter_ustb(nd) + ct*2*&
&       rotor_diameter_ust*tempb14(nd) + ct*2*rotor_diameter_ust*tempb17&
&       (nd)
      sigmay_db(nd) = sigmaz_d*8.0_dp*tempb18(nd)
      sigmaz_db(nd) = 8.0_dp*sigmay_d*tempb18(nd)
      tempb15(nd) = -(temp16*tempb14(nd))
      sigmay_0b(nd) = sigmaz_0*8.0_dp*tempb15(nd)
      sigmaz_0b(nd) = 8.0_dp*sigmay_0*tempb15(nd)
    END DO
    discontinuity_pointb(:) = 0.0_8
  ELSE
    STOP
...
! Niayifar and Porte Agel 2015, 2016
    ...
    DO nd=1,nbdirs
      IF (ti_dst_in**2.0_dp + temp4**2.0_dp .EQ. 0.0) THEN
        tempb2(nd) = 0.0
      ELSE
        tempb2(nd) = ti_dstb(nd)/(2.0*SQRT(ti_dst_in**2.0_dp+temp4**&
&         2.0_dp))
      END IF
      tempb3(nd) = 2.0_dp*temp4*tempb2(nd)/rotor_area_dst
      ti_dst_inb(nd) = 2.0_dp*ti_dst_in*tempb2(nd)
      ti_addedb(nd) = wake_overlap*tempb3(nd)
      wake_overlapb(nd) = ti_added*tempb3(nd)
      rotor_area_dstb(nd) = -(temp4*tempb3(nd))
      rotor_diameter_dstb(nd) = rotor_diameter_dstb(nd) + pi*0.25_dp*&
&       2_dp*rotor_diameter_dst*rotor_area_dstb(nd)
      tempb0(nd) = 0.73_dp*temp1**(-0.32_dp)*ti_addedb(nd)
      tempb1(nd) = -(0.32_dp*temp1**(-1.32)*temp3*temp2*0.73_dp*&
&       ti_addedb(nd)/rotor_diameter_ust)
      axial_induction_ustb(nd) = temp2*0.8325_dp*axial_induction_ust**(&
&       -0.1675)*tempb0(nd)
      ti_ustb(nd) = temp3*0.0325_dp*ti_ust**(-0.9675)*tempb0(nd)
    END DO
...
&                       wake_overlap, wake_overlapb, nbdirs)
    DO nd=1,nbdirs
      ...
      xb(nd) = k_star_ust*sigmab(nd) + tempb1(nd)
...
&         temp0+1.0_dp)/(2.0*temp**2)-1.0/(2.0*temp0))*tempb(nd)
    END DO
...
! Check if this is the max and use it if it is
    ...
    DO nd=1,nbdirs
      IF (ti**2.0_dp + temp10**2.0_dp .EQ. 0.0) THEN
        tempb7(nd) = 0.0
      ELSE
        tempb7(nd) = 2.0_dp*temp10*ti_tmpb(nd)/(2.0*SQRT(ti**2.0_dp+&
&         temp10**2.0_dp)*rotor_area_dst)
      END IF
      ti_addedb(nd) = wake_overlap*tempb7(nd)
      wake_overlapb(nd) = ti_added*tempb7(nd)
      rotor_area_dstb(nd) = -(temp10*tempb7(nd))
      rotor_diameter_dstb(nd) = rotor_diameter_dstb(nd) + pi*0.25_dp*&
&       2_dp*rotor_diameter_dst*rotor_area_dstb(nd)
...
&       (nd) - temp7*tempb6(nd)
      ...
      ELSE
        betab(nd) = 0.2_dp*epsilonb(nd)/(2.0*SQRT(beta))
      END IF
      tempb4(nd) = 0.5_dp*betab(nd)/temp5
      IF (.NOT.1.0_dp - ct_ust .EQ. 0.0) ct_ustb(nd) = ct_ustb(nd) + ((&
&         temp6+1.0_dp)/(2.0*temp5**2)-1.0/(2.0*temp6))*tempb4(nd)
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
    ti_dst_inb(:) = 0.0_8
...
&                ti_tmpb, ti_dst, ti_dstb, nbdirs)
    ...
    DO nd=1,nbdirs
      IF (ti**2.0_dp + temp16**2.0_dp .EQ. 0.0) THEN
        tempb11(nd) = 0.0
      ELSE
        tempb11(nd) = 2.0_dp*temp16*ti_tmpb(nd)/(2.0*SQRT(ti**2.0_dp+&
&         temp16**2.0_dp)*rotor_area_dst)
      END IF
      ti_addedb(nd) = wake_overlap*tempb11(nd)
      wake_overlapb(nd) = ti_added*tempb11(nd)
      rotor_area_dstb(nd) = -(temp16*tempb11(nd))
      rotor_diameter_dstb(nd) = rotor_diameter_dstb(nd) + pi*0.25_dp*&
&       2_dp*rotor_diameter_dst*rotor_area_dstb(nd)
...
&       (nd) - temp13*tempb10(nd)
      ...
      ELSE
        betab(nd) = 0.2_dp*epsilonb(nd)/(2.0*SQRT(beta))
      END IF
      tempb8(nd) = 0.5_dp*betab(nd)/temp11
      IF (.NOT.1.0_dp - ct_ust .EQ. 0.0) ct_ustb(nd) = ct_ustb(nd) + ((&
&         temp12+1.0_dp)/(2.0*temp11**2)-1.0/(2.0*temp12))*tempb8(nd)
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
    ti_dstb(:) = 0.0_8
...
&             ti_area_ratio**2.0_dp))
      ...
      DO nd=1,nbdirs
        tempb15(nd) = ti_area_ratio_tmpb(nd)/rotor_area_dst
        ti_addedb(nd) = wake_overlap*tempb15(nd)
        wake_overlapb(nd) = ti_added*tempb15(nd)
        rotor_area_dstb(nd) = -(ti_added*wake_overlap*tempb15(nd)/&
&         rotor_area_dst)
        rotor_diameter_dstb(nd) = rotor_diameter_dstb(nd) + pi*0.25_dp*&
&         2_dp*rotor_diameter_dst*rotor_area_dstb(nd)
...
&         sigmab(nd) - temp19*tempb14(nd)
        ...
        ELSE
          betab(nd) = 0.2_dp*epsilonb(nd)/(2.0*SQRT(beta))
        END IF
        tempb12(nd) = 0.5_dp*betab(nd)/temp17
        IF (.NOT.1.0_dp - ct_ust .EQ. 0.0) ct_ustb(nd) = ct_ustb(nd) + (&
&           (temp18+1.0_dp)/(2.0*temp17**2)-1.0/(2.0*temp18))*tempb12(nd&
&           )
      END DO
      CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust&
&                            , axial_induction_ustb, nbdirs)
...
&                    nbdirs)
        ...
        DO nd=1,nbdirs
          tempb19(nd) = ti_area_ratio_tmpb(nd)/rotor_area_dst
          ti_addedb(nd) = wake_overlap*tempb19(nd)
          wake_overlapb(nd) = ti_added*tempb19(nd)
          rotor_area_dstb(nd) = -(ti_added*wake_overlap*tempb19(nd)/&
&           rotor_area_dst)
          rotor_diameter_dstb(nd) = rotor_diameter_dstb(nd) + pi*0.25_dp&
&           *2_dp*rotor_diameter_dst*rotor_area_dstb(nd)
          tempb17(nd) = 0.73_dp*temp24**(-0.32_dp)*ti_addedb(nd)
          tempb18(nd) = -(0.32_dp*temp24**(-1.32)*temp26*temp25*0.73_dp*&
&           ti_addedb(nd)/rotor_diameter_ust)
          axial_induction_ustb(nd) = temp25*0.8325_dp*&
&           axial_induction_ust**(-0.1675)*tempb17(nd)
      ...
      ELSE
        ...
        DO nd=1,nbdirs
          ...
          IF (ovr**2 - (ovdyd-ovl)**2 .EQ. 0.0) THEN
            tempb3(nd) = 0.0
          ELSE
            tempb3(nd) = ovz2b(nd)/(2.0*SQRT(ovr**2-(ovdyd-ovl)**2))
          END IF
          tempb4(nd) = -(2*(ovdyd-ovl)*tempb3(nd))
          IF (ovrr**2 - ovl**2 .EQ. 0.0) THEN
            tempb2(nd) = 0.0
          ELSE
            tempb2(nd) = ovzb(nd)/(2.0*SQRT(ovrr**2-ovl**2))
          END IF
          IF (temp .EQ. 1.0 .OR. temp .EQ. (-1.0)) THEN
            tempb5(nd) = 0.0
          ELSE
            tempb5(nd) = -(ovrr*wake_overlapb(nd)/SQRT(1.0-temp**2))
          END IF
          IF (temp0 .EQ. 1.0 .OR. temp0 .EQ. (-1.0)) THEN
            tempb6(nd) = 0.0
          ELSE
            tempb6(nd) = -(ovr*wake_overlapb(nd)/SQRT(1.0-temp0**2))
          END IF
          ovlb(nd) = (ovz2-ovz)*wake_overlapb(nd) - 2*ovl*tempb2(nd) - &
&           tempb4(nd) - tempb6(nd) + tempb5(nd)
          tempb1(nd) = ovlb(nd)/(2.0_dp*ovdyd)
          ovrrb(nd) = 2*ovrr*tempb2(nd) + 2*ovrr*tempb1(nd) - temp*&
&           tempb5(nd) + ACOS(temp)*2*ovrr*wake_overlapb(nd)
          ovrb(nd) = 2*ovr*tempb3(nd) - 2*ovr*tempb1(nd) - temp0*tempb6(&
&           nd) + ACOS(temp0)*2*ovr*wake_overlapb(nd)
          ovdydb(nd) = tempb4(nd) + (2*ovdyd-(ovrr**2-ovr**2+ovdyd**2)/&
&           ovdyd)*tempb1(nd) - ovz2*wake_overlapb(nd) + tempb6(nd)
        END DO
      END IF
    ELSE
      IF (branch .EQ. 3) THEN
...
! distance from rotor to the last point where the wake model is undefined
  ...
  temp0 = SQRT(temp)
  DO nd=1,nbdirs
    tempb1(nd) = rotor_diameter*discontinuity_pointb(nd)/c
    tempb2(nd) = (a-temp0)*discontinuity_pointb(nd)/c
    x0b(nd) = x0b(nd) + discontinuity_pointb(nd)
    IF (temp .EQ. 0.0) THEN
      ab(nd) = tempb1(nd)
    ELSE
      ab(nd) = (1.0_8-2*a/(2.0*temp0))*tempb1(nd)
    END IF
    IF (temp .EQ. 0.0) THEN
      bb(nd) = 0.0
    ELSE
      bb(nd) = tempb1(nd)/(2.0*temp0)
    END IF
    rotor_diameterb(nd) = rotor_diameterb(nd) + tempb2(nd)
    cb(nd) = -(rotor_diameter*tempb2(nd)/c)
    tempb0(nd) = SQRT(8.0_dp)*2.0_dp*cb(nd)
    tempb(nd) = 4.0_dp*COS(yaw)*bb(nd)
    kyb(nd) = kyb(nd) + (ct-1.0_dp)*kz*tempb(nd) + ab(nd) + kz*tempb0(nd&
&     )
    kzb(nd) = kzb(nd) + (ct-1.0_dp)*ky*tempb(nd) + COS(yaw)*ab(nd) + ky*&
&     tempb0(nd)
