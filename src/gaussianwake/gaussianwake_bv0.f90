...
! print TIturbs values to a file
  ...
  DO nd=1,nbdirs
    ...
    ky_localb(nd, :) = 0.0_8
  END DO
  print *, "before d turb loop, ctb is ", ct_localb
  DO d=nturbines,1,-1
    print *, "starting d turb loop ", d, " ctb is ", ct_localb
    CALL POPCONTROL1B(branch)
    IF (branch .NE. 0) THEN
      turbi = sorted_x_idx(d) + 1
      CALL POPREAL8(ct_local(turbi))
!       print *, "after interpolation_bv func, ctb is ", ct_localb
...
&                     nbdirs)
      print *, "after interpolation_bv func, ctb is ", ct_localb
    END IF
    print *, "after interpolation_bv func logic, ctb is ", ct_localb
    DO nd=1,nbdirs
      ...
      DO u=nturbines,1,-1
        ...
        ELSE
          ...
          ELSE
            ...
            print *, "before added_ti, line 388, ctb is ", ct_localb
...
&                           titurbs(turbi), titurbsb(1, turbi), nbdirs)
            print *, "after added ti, line 399, ctb is ", ct_localb
...
&                                 , deficit_sumb, nbdirs)
          ...
          IF (branch .EQ. 0) THEN
            ...
            print *, "before deltav, ctb is ", ct_localb
...
&                         , deltavb, nbdirs)
            print *, "after deltav, ctb is ", ct_localb
            DO nd=1,nbdirs
          ...
          ELSE
            deltaz = localrotorpointz + turbinez(turbi) - turbinez(turb)
            CALL POPREAL8(deltav)
            print *, "before deltav near wake, ctb is ", ct_localb
...
&                                       deltav, deltavb, nbdirs)
            print *, "after deltav near wake, ctb is ", ct_localb
...
&               deltax0_dpb(nd)
          ...
          print *, "before discontinuity point, ctb is ", ct_localb
...
&                                    discontinuity_pointb, nbdirs)
          print *, "after discontinuity point, ctb is ", ct_localb
...
&             (turbi))*deltayb(nd)
          ...
          END DO
          print *, "before wake_offset, ctb is ", ct_localb
...
&                            nbdirs)
          print *, "after wake offset, ctb is ", ct_localb
...
&                         , k_starb, nbdirs)
          ...
          CALL POPREAL8(x0)
          print *, "before x0, ctb is ", ct_localb
...
&                   beta, x0, x0b, nbdirs)
          print *, "after x0, ctb is ", ct_localb
        END IF
        CALL POPREAL8(x)
        DO nd=1,nbdirs
...
&         *0.5_dp*localrotorpointzb(nd)
  ...
  print *, "ending d turb loop ", d, " ctb is ", ct_localb
  END DO
  DO nd=1,nbdirs
    ctb(nd, :) = 0.0_8
    ctb(nd, :) = ct_localb(nd, :)
...
! intrinsic functions
  ...
  print *, ti, ct_ust, ct_ustb, x, xb, k_star_ust, &
& k_star_ustb, rotor_diameter_ust, rotor_diameter_ustb, &
& rotor_diameter_dst, rotor_diameter_dstb, deltay, deltayb, wake_height&
& , wake_heightb, turbine_height, turbine_heightb, ti_ust, ti_ustb, &
& ti_calculation_method, ti_area_ratio_in, ti_dst_in, ti_dst_inb, &
& ti_area_ratio
  ...
  print *, "entering added ti, line 1454, ct_ustb is ", ct_ustb
...
&         temp5+1.0_dp)/(2.0*temp6**2)-1.0/(2.0*temp5))*tempb7(nd)
      print *, "line 1620, ct_ustb is ", ct_ustb
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
    print *, "line 1624, ct_ustb is ", ct_ustb
    DO nd=1,nbdirs
      ti_dst_inb(nd) = 0.0_8
    END DO
...
&         temp11+1.0_dp)/(2.0*temp12**2)-1.0/(2.0*temp11))*tempb11(nd)
      print *, "line 1710, ct_ustb is ", ct_ustb
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
    print *, "line 1714, ct_ustb is ", ct_ustb
    DO nd=1,nbdirs
      ti_dstb(nd) = 0.0_8
    END DO
...
&         temp17+1.0_dp)/(2.0*temp18**2)-1.0/(2.0*temp17))*tempb15(nd)
      print *, "line 1796, ct_ustb is ", ct_ustb
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
    print *, "line 1800, ct_ustb is ", ct_ustb
    DO nd=1,nbdirs
      ti_dst_inb(nd) = 0.0_8
    END DO
...
&       -0.1675)*tempb17(nd)
    ...
    END DO
    ! print *, "line 1858 ", deltay, deltayb, turbine_height, &
! &                       turbine_heightb, rotor_diameter_dst, &
! &                       rotor_diameter_dstb, 0.0_dp, wake_height, &
! &                       wake_heightb, wake_diameter, wake_diameterb, &
! &                       wake_overlap, wake_overlapb, nbdirs
...
&                       wake_heightb, wake_diameter, wake_diameterb, &
&                       wake_overlap, wake_overlapb, nbdirs)
    ! print *, "line 1868 ", deltay, deltayb, turbine_height, &
! &                       turbine_heightb, rotor_diameter_dst, &
! &                       rotor_diameter_dstb, 0.0_dp, wake_height, &
! &                       wake_heightb, wake_diameter, wake_diameterb, &
! &                       wake_overlap, wake_overlapb, nbdirs
...
!       print *, "wake_diameterb ", wake_diameterb
      sigmab(nd) = 4.0_dp*wake_diameterb(nd)
!       print *, "sigmab line 1865 ", sigmab
      xb(nd) = k_star_ust*sigmab(nd) + tempb18(nd)
      rotor_diameter_ustb(nd) = rotor_diameter_ustb(nd) + epsilon*sigmab&
&       (nd) - temp25*tempb18(nd)
      k_star_ustb(nd) = k_star_ustb(nd) + x*sigmab(nd)
      print *, rotor_diameter_ust
!       print *, "sigmab ", sigmab
      epsilonb(nd) = rotor_diameter_ust*sigmab(nd)
...
!       print *, "temp23 ", temp23
!       print *, "temp24 ", temp24
!       print *, "line 1879, ct_ustb is ", ct_ustb
      print *, (temp23+1.0_dp)/(2.0*temp24**2)
      print *, 1.0/(2.0*temp23)
      print *, ((temp23+1.0_dp)/(2.0*temp24**2)-1.0/(2.0*temp23))
      print *, tempb19
      IF (.NOT.1.0_dp - ct_ust .EQ. 0.0) ct_ustb(nd) = ct_ustb(nd) + ((&
&         temp23+1.0_dp)/(2.0*temp24**2)-1.0/(2.0*temp23))*tempb19(nd)
!       print *, "line 1882, ct_ustb is ", ct_ustb
    END DO
    CALL CT_TO_AXIAL_IND_FUNC_BV(ct_ust, ct_ustb, axial_induction_ust, &
&                          axial_induction_ustb, nbdirs)
!     print *, "line 1886, ct_ustb is ", ct_ustb
    DO nd=1,nbdirs
      ti_dstb(nd) = 0.0_8
    END DO
  ELSE
...
! !  Differentiation of smooth_max in reverse (adjoint) mode:
! !   gradient     of useful results: g
! !   with respect to varying inputs: g x y
! !   RW status of diff variables: g:in-zero x:out y:out
! SUBROUTINE SMOOTH_MAX_BV(x, xb, y, yb, g, gb, nbdirs)
!   !USE DIFFSIZES
