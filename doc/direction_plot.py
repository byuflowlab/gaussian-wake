...
if __name__ == "__main__":
    ...
    gauss_P1 = np.zeros(np.size(directions))
    gauss_P2 = np.zeros(np.size(directions))

    floris_AEP = np.zeros(np.size(directions))
    floris_P1 = np.zeros(np.size(directions))
    floris_P2 = np.zeros(np.size(directions))
    i = 0

    for d in directions:
        gauss_prob['windDirections'] = np.array([d])
        gauss_prob.run()
        gauss_AEP[i] = gauss_prob['AEP']
        gauss_P1[i] = gauss_prob['wtPower0'][0]
        gauss_P2[i] = gauss_prob['wtPower0'][1]

        floris_prob['windDirections'] = np.array([d])
        floris_prob.run()
